...
{  tree_node_t *tmp_node;
   ...
   {  tree->left = (tree_node_t *) new_object;
	...
	tmp_node->weight = 2;
      ...
      printf("\nstack is: ");
      while(x < path_st_p){
	printf("%d ",path_stack[x]->key);
	x++;
      }
      printf("\n");
      printf("Tree is: \n");
      check_tree(tree,0,-1000,1000);
      printf(" Over \n");
      /* rebalance  Weight Balance Tree 3.2 */
      while( path_st_p > 0 ){
	tmp_node = path_stack[--path_st_p];
	tmp_node->weight =  tmp_node->left->weight  + tmp_node->right->weight;
	if( tmp_node->right->weight < ALPHA * tmp_node->weight ){/*Case 2: */
	  if( tmp_node->left->left->weight > (ALPHA+EPSILON)*tmp_node->weight ){/*Case 2.1: */
	...
	else if ( tmp_node->left->weight < ALPHA*tmp_node->weight){/* Case 3: */
	  if( tmp_node->right->right->weight > (ALPHA+EPSILON)*tmp_node->weight ){/* Case 3.1:  */
	...
	if( tmp_node->right->weight < ALPHA*tmp_node->weight){
	  if( tmp_node->left->left->weight > (ALPHA+EPSILON)*tmp_node->weight ){  
	    ...
	    tmp_node->left->weight =  tmp_node->left->left->weight + tmp_node->left->right->weight;
          ...
          if( tmp_node->right->right->weight > (ALPHA+EPSILON)*tmp_node->weight ){
	    left_rotation( tmp_node );
	    tmp_node->left->weight = tmp_node->left->left->weight + tmp_node->left->right->weight;
	  }
	  else{
